                          ...
                          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10, marginBottom: 16 }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 11, flex: 1, minWidth: 0 }}>
                              ...
                              <div style={{ minWidth: 0, flex: 1 }}>
                                <div style={{ display: "flex", alignItems: "center", gap: 7, fontFamily: "'Space Grotesk'", fontWeight: 600, fontSize: 16 }}><span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", minWidth: 0 }}>{p.confSymbol}</span><span style={{ flexShrink: 0, fontSize: 11, color: "var(--violet)", background: "var(--violet-dim)", padding: "2px 7px", borderRadius: 6, fontFamily: "'Instrument Sans'", fontWeight: 600 }}>ERC-7984</span></div>
                                <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12.5, color: "var(--muted)" }}><span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", minWidth: 0 }}>{p.name}</span>{p.official ? <span title="Official Zama cTokenMock" style={{ flexShrink: 0, fontSize: 10.5, color: "var(--accent-ink)", background: "var(--accent)", padding: "1px 6px", borderRadius: 5, fontFamily: "'Instrument Sans'", fontWeight: 700 }}>✓ Official</span> : p.custom ? <span title="Custom pair you added — saved in this browser" style={{ flexShrink: 0, fontSize: 10.5, color: "var(--violet)", background: "var(--violet-dim)", padding: "1px 6px", borderRadius: 5, fontFamily: "'Instrument Sans'", fontWeight: 700 }}>Custom</span> : <span title="Third-party pair registered onchain — not part of Zama's official set" style={{ flexShrink: 0, fontSize: 10.5, color: "var(--muted)", background: "var(--surface2)", border: "1px solid var(--border)", padding: "1px 6px", borderRadius: 5, fontFamily: "'Instrument Sans'", fontWeight: 600 }}>Community</span>}{p.custom && <button onClick={() => removePair(idOf(p))} title="Remove custom pair" style={{ flexShrink: 0, marginLeft: 2, padding: "0 5px", borderRadius: 5, cursor: "pointer", border: "1px solid var(--border)", background: "transparent", color: "var(--muted)", fontSize: 11, fontFamily: "'Instrument Sans'", lineHeight: 1.6 }}>✕</button>}</div>
                              </div>
                            </div>
                            <div title={`Underlying ${p.decimals} decimals → confidential ${p.confDecimals ?? confDecimalsOf(p.decimals ?? 18)} decimals. Conversion rate ${formatRate(p.rate)} (base units).`} style={{ flexShrink: 0, textAlign: "right", fontFamily: "'JetBrains Mono'", lineHeight: 1.45 }}>
